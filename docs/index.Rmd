...
```{r pressure, echo=FALSE, results = "hide", message = FALSE, warnings = FALSE}
...
plot2_data <- dp_year_data %>% 
  group_by(hb_name, hb) %>% 
  summarise(paid_quantity = sum(paid_quantity)) %>% 
  left_join(., deprivation_data, by = join_by(hb == hb)) %>% 
  left_join(., NHS_healthboards, by = join_by(hb == HBCode))
```
...
ggplot(aes(geometry = geometry, fill = paid_quantity)) +
  ...
  geom_sf_text(aes(label = HBName), size = 2, color = "black", fontface = "bold")  # Adjust size and color of the text
