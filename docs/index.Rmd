...
stroke_mortality_data <- read_csv("https://www.opendata.nhs.scot/dataset/f5dcf382-e6ca-49f6-b807-4f9cc29555bc/resource/285b4cbd-1e87-49fe-9e24-73d2da8166ae/download/stroke_mortalitybyca.csv") %>% 
  ...
  rename(stroke = number_of_deaths)
...
cardiovascular_mortality_data <- read_csv("https://www.opendata.nhs.scot/dataset/0e17f3fc-9429-48aa-b1ba-2b7e55688253/resource/fc7b42f1-4de6-48dd-b640-613fb0aa427d/download/hd_mortalitybyca.csv") %>% 
  ...
  group_by(ca) %>% 
  summarise(number_of_deaths = sum(number_of_deaths, na.rm = TRUE)) %>% 
  rename(cardiovascular = number_of_deaths)

mortality_data <- full_join(stroke_mortality_data, cardiovascular_mortality_data)
```
...
gt_data <- weightdrug_data %>%
  ...
  summarise(total_paid_quantity = sum(paid_quantity, na.rm = TRUE)/head(hb_population, 1) * 100000,
            ...
            cardiovascular = mean(stroke, na.rm = TRUE)) %>%
  arrange(drug_category) 
## hb_name was removed from both the group by and arrange functions

#gt_data <- gt_data %>% 
#  select(drug_category:mortality_deaths)
#Use ifelse or casewhen to get a count of each weight loss drug
...
gt_data %>%  
 ...
 # summary_rows(columns = c(total_paid_quantity, ar_simd, stroke, cardiovascular),
  #             fns = list(Average = ~round(mean(., na.rm = TRUE), 2))) %>% 
 # grand_summary_rows(columns = c(total_paid_quantity, ar_simd, stroke, cardiovascular), 
  #                   fns = list("Overall Average" = ~round(mean(., na.rm = TRUE), 2))) %>% 
