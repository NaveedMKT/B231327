...
population_data <- read_csv(here( "data", "UV103_age_health_board_census.csv"), skip = 10) %>% 
  ...
  NHS_healthboards <- st_read(here("data", "NHS_healthboards_2019.shp")) %>% 
    clean_names() #Loads in the data for the creation of the map
...
gp_data <- read_csv("https://www.opendata.nhs.scot/dataset/f23655c3-6e23-4103-a511-a80d998adb90/resource/64b42f68-3353-449b-bc1c-0f733b21803b/download/practice_contactdetails_oct2024-open-data.csv") %>% 
  clean_names() %>% #Loads in the data for each GP so as to connect the prescriptions and datazone
...
map_data <- weightdrug_data %>% 
  group_by(hb) %>% 
  summarise(paid_quantity = sum(paid_quantity)) %>% #Sum of prescriptions per health board
  left_join(., NHS_healthboards, by = join_by(hb == hb_code)) 
#Adds in information to plot the map for prescriptions for each health board
...
ggplot(aes(geometry = geometry, fill = paid_quantity)) +
  ...
  annotation_north_arrow(
    ...
    style = north_arrow_nautical(
      fill = c("blue", "white"),      
      line_col = "grey20"
    ) #Adds in the North Arrow
  ) +
  geom_sf_text(aes(label = hb_name), size = 2, color = "black", fontface = "bold")  #Adds in labels for each health board
```
...
dp_plot_data %>% 
  ...
  labs(title = "Prescriptions of Diabetes Drugs vs average SIMD",
...
```{r}
str_mortality_plot_data_long <- str_mortality_plot_data %>%
  pivot_longer(cols = c(stroke, cardiovascular), 
               names_to = "category", 
               values_to = "value") %>% 
  mutate(category = case_when(category == "stroke" ~ "Stroke Deaths",
                              category == "cardiovascular" ~ "Cardiovascular Deaths"))


str_mortality_plot_data_long %>% 
...
mortality_lm_model <- lm(paid_quantity ~ stroke+cardiovascular, data = str_mortality_plot_data)
cor.test(str_mortality_plot_data$paid_quantity, (str_mortality_plot_data$cardiovascular + str_mortality_plot_data$stroke))
...
gt_data <- weightdrug_data %>%
  ...
  summarise(total_paid_quantity = sum(paid_quantity, na.rm = TRUE)/head(hb_population, 1) * 100000,
            ...
            stroke = mean(stroke, na.rm = TRUE)/head(hb_population, 1) * 100000,
            cardiovascular = mean(cardiovascular, na.rm = TRUE)/head(hb_population, 1) * 100000,
            drug_category = drug_category [which.max(total_paid_quantity)])

gt_data %>%  
  gt() %>% 
     cols_label(
       drug_category = "Most Prescribed Type of Diabetes Drug", 
 ...
 grand_summary_rows(columns = c(total_paid_quantity, ar_simd, stroke, cardiovascular), fns = list("Overall Average" = ~round(mean(., na.rm = TRUE), 2))) %>% 
     fmt_number(columns = c(total_paid_quantity, ar_simd, stroke, cardiovascular), decimals = 2) %>% 
     ...
     tab_spanner(label = "Rate per 100k",
              columns = c(total_paid_quantity, stroke, cardiovascular))
```
